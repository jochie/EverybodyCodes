...
def parse_options():
    ...
    parser = argparse.ArgumentParser(
        description='This program is used for one of the EverybodyCodes 2024 puzzles; Day 5, part 2'
    )
...
def main():
    ...
    while True:
        ...
        if number not in shouts:
            shouts[number] = 0
        shouts[number] += 1
        print(f"Number for round {iteration}: {number}   Shouts: {shouts[number]}")
        if shouts[number] == 2024:
            print(f"Multiplying: {int(number) * iteration}")
            break
        print()
        iteration += 1
